...
const ProductCard = ({ id, image, images, name, description, price, seller, sellerId, shopId, shopLogoUrl, index }: ProductCardProps) => {
  ...
  const handleContact = async (e: React.MouseEvent) => {
    ...
    if (user.id === sellerId) { toast({ title: t("ownProduct") }); return; }
    try {
      const { data: productRow } = await supabase
        .from("products").select("contact_whatsapp").eq("id", id).maybeSingle();
      if (productRow?.contact_whatsapp) {
        const num = productRow.contact_whatsapp.replace(/[^\d+]/g, "").replace(/^\+/, "");
        const text = `Bonjour, je suis intéressé(e) par votre produit: ${name}${allImages[0] ? `\n${allImages[0]}` : ""}`;
    ...
    try {
      ...
      if (existing) navigate("/messages");
      else {
        await supabase.from("conversations").insert({ participant_1: p1, participant_2: p2, product_id: id });
        navigate("/messages");
      }
    } catch { toast({ title: t("error"), variant: "destructive" }); }
  };

  const openShop = () => { if (shopId) setShopOpen(true); };
  ...
  return (
    <>
      <motion.div
        ...
        onClick={openShop}
        className="group bg-card rounded-xl overflow-hidden border border-border hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer"
      >
        <div className="relative aspect-square overflow-hidden bg-muted">
          ...
          <button
            ...
            className="absolute top-3 left-3 w-9 h-9 rounded-full overflow-hidden border-2 border-card bg-card shadow-md flex items-center justify-center"
            title={seller}
          >
            {shopLogoUrl ? (
              <img src={shopLogoUrl} alt={seller} className="w-full h-full object-cover" />
            ) : (
      ...
      {shopId && (
        <ShopSheet open={shopOpen} onOpenChange={setShopOpen} shopId={shopId} sellerId={sellerId} />
      )}
    </>
  );
};
